...
- **Node.js v22+** -- needed for `modules/pwa`'s TypeScript service worker (compiled via `js.Build`), the project's ESLint/Prettier setup, and the validation suites under `modules/pwa/test/`, `modules/social-share/test/`, and `modules/search/test/` (all Playwright) plus `modules/images/test/` and `modules/agent-readiness/test/` (Node build-output assertions).
