...
- **Hugo v0.160.0+** (any edition) -- matching every module's `[module.hugoVersion]` floor.
- **Go 1.22+** -- required by every leaf `go.mod`.
- **Node.js v22+** -- needed for `modules/pwa`'s TypeScript service worker (compiled via `js.Build`), the project's ESLint/Prettier setup, and the validation suites under `modules/pwa/test/`, `modules/social-share/test/`, `modules/search/test/`, `modules/copy-page/test/`, and `modules/carousel/test/` (all Playwright) plus `modules/images/test/`, `modules/seo/test/`, `modules/agent-readiness/test/`, `shortcodes/github-profile/test/`, `shortcodes/accordion/test/`, `modules/og-image/test/`, `modules/url-retirement/test/`, `shortcodes/test-smoke/`, and `modules/test-composition/` (Node build-output assertions; several build their fixture more than once -- github-profile plain, minified and origin-backed, accordion at Hugo's default Markdown settings and again with raw HTML allowed -- and five need no `npm install` of their own: `og-image` because its assertions read published rasters with `node:zlib` alone, `url-retirement` and `accordion` because theirs read published text with Node's own modules, and the last two because they are suites rather than modules, one covering the five shortcode modules that ship no suite of their own and one cross-module).
...
All templates, configuration, and code MUST be compatible with **Hugo v0.160.1+**. Do not use functions, methods, template constructs, configuration keys, or CLI flags that are deprecated at v0.160.1 or earlier. Use the modern replacements (for example, Dart Sass instead of embedded LibSass; `.Meta` instead of `.Exif`; `hugo.Data` instead of `.Site.Data`). Validate with `hugo build --logLevel info | grep deprecate` against a consuming site -- zero deprecation warnings is the bar, and it is not the whole bar: a few spellings are retired in Hugo's documentation while its code stays silent, so a clean build says nothing about them. `.Language.Lang` (use `.Language.Name`) is one of those, and `npm run check:hugo-api` is what catches it across every template here.
...
5. When the module set changes, keep the surfaces that enumerate modules in sync so they do not silently go stale: the root [`README.md`](README.md) Modules and Shortcodes sections, and the `.github/ISSUE_TEMPLATE/` dropdowns (`bug-report.yml` Affected Module, `feature-request.yml` Target Module, `question.yml` Topic Area, and `docs-issue.yml` Affected Documentation for the new module's README). Each issue form carries an inline `# NOTE` at the relevant dropdown as a reminder. Do this in the same change as adding or removing the module -- a stale dropdown ships a module a user cannot select, or lists one that no longer exists. When a module's `[outputFormats]` table changes rather than the module set, a different sweep applies: the cross-module suite derives the required format set from those tables, so the merged `[outputs] home` example in every module README that shows one, the merged list in `modules/test-composition/fixture/hugo.toml`, and `modules/test-composition/README.md` all have to move in the same change or the blocking suite goes red in a module you did not touch.
...
- Run `npm run check` (ESLint, Prettier, markdownlint, the module-pin check, the retired-Hugo-API check, and the store-map-read check) and fix anything it reports before submitting. CI runs those six plus two gates with no npm equivalent, because neither tool is a Node package: `yamllint -c .yamllint.yml --strict .` and `editorconfig-checker` (installable with `go install github.com/editorconfig-checker/editorconfig-checker/v3/cmd/editorconfig-checker@v3.0.3`). Run those two directly when you touch YAML, or let CI catch them.
- Run the test suite of every module you changed, with `bash <module>/test/run-tests.sh` (`modules/pwa/test/run-matrix.sh` for the PWA matrix). CI runs all of them, one job per suite, so a suite you have not run is a suite you will see fail on the pull request instead of on your machine.
