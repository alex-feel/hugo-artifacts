...
- **Node.js v22+** -- needed for `modules/pwa`'s TypeScript service worker (compiled via `js.Build`), the project's ESLint/Prettier setup, and the validation suites under `modules/pwa/test/`, `modules/social-share/test/`, `modules/search/test/`, `modules/copy-page/test/`, and `modules/carousel/test/` (all Playwright) plus `modules/images/test/`, `modules/seo/test/`, `modules/agent-readiness/test/`, `shortcodes/github-profile/test/`, `modules/og-image/test/`, `modules/url-retirement/test/`, `shortcodes/test-smoke/`, and `modules/test-composition/` (Node build-output assertions; the github-profile suite is the only one that builds its fixture twice, once with `--minify`, and the last four need no `npm install` of their own -- `og-image` because its assertions read published rasters with `node:zlib` alone, `url-retirement` because its assertions read published text files with Node's own modules, and the other two because they are suites rather than modules, one covering the five shortcode modules that ship no suite of their own and one cross-module).
