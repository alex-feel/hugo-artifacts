...
Node build-output assertions for `modules/seo`, run against the static HTML that ten Hugo builds of [`fixture/`](fixture/) produce. The module ships zero JavaScript, so there is no browser behavior to test and the suite carries no Playwright dependency.

## Why ten builds

The suite builds the fixture ten times, and every build is load-bearing:
...
- **`layouts/_partials/fixture/og-card.html` and `assets/img/card-base.png`**, used only by the `generated` environment: a stand-in generated-image module that draws the resolved title onto a template raster with `images.Text` and publishes the result per page, so the hook is exercised with a real composed Resource rather than a path string.
...
| `tests/11-generated-image.spec.js` | The generated-image hook: a composed card outranks `default_image` and reaches both `og:image` and JSON-LD with real dimensions, a page carrying its own image gets no card on any surface, a page the generator declines falls through to the site banner, an author's `Person.image` is never a card, the hook is handed the resolved title rather than the raw one, and a missing or non-scalar `image_partial` each warn once and change nothing else. |
