...
A site importing this module together with [`agent-readiness`](../agent-readiness/README.md), [`search`](../search/README.md), [`pwa`](../pwa/README.md) and [`url-retirement`](../url-retirement/README.md) wires all of them at once:
...
**Out of scope (the module deliberately never emits these):** `<meta charset>`, viewport, favicons, `apple-touch-icon`, `theme-color`, `<html lang>`, `<base href>`, `<meta name="keywords">`, the XML sitemap, `X-Robots-Tag` headers, `robots.txt`, and `llms.txt`. Those belong to your base layout or to Hugo itself. `<base href>` is never emitted by design (it breaks relative URLs); the module relies on absolute `.Permalink` throughout. `robots.txt`-level AI training and citation controls are out of scope, since the module emits no `robots.txt`; the meta-robots route is in scope, and Bing's `noarchive` and `nocache` tokens pass through from your robots configuration. Only `seo/head.html`'s "pass the page, get the complete head SEO surface" contract is the stable public API; internal partials may change between minor versions, though same-name overrides are supported and documented.
...
seo:
  video: # NEW; presence auto-detects VideoObject (Hugo has no native video metadata)
    ...
    clips: # -> hasPart[] Key Moments (name, start_offset, and url required per clip; unique start_offsets; mutually exclusive with seek_url_template)
...
- **Images:** `og:image`/`twitter:image` and the page-level JSON-LD `image[]` use a content-aware 1200x630 (1.91:1) smart crop with real dimensions; entity images -- `Organization.logo`, ProfilePage `mainEntity.image`, and author `Person.image` -- keep the source image's native aspect, so a logo or portrait is never force-cropped. An image that ALREADY measures 1200x630 is used as it is: cropping it would write a second file holding the same pixels, and it reaches `og:image` exactly as you exported it rather than re-encoded. Only an image a page actually names is written to `public/` -- a candidate that loses the precedence race, and a native-aspect variant no node asks for, cost nothing.
...
**Supported pass-through tokens:** `noindex`, `nofollow`, `none`, `nosnippet`, `noimageindex`, `notranslate`, `indexifembedded` (only meaningful alongside `noindex`), `noarchive` and `nocache` (Google retired both, but Bing repurposed them in September 2023: `noarchive` keeps a page out of Bing Chat answers and out of training Microsoft's generative-AI models, `nocache` allows the URL, title and snippet only, and Bing resolves a page carrying both as `nocache`), `max-snippet:N`, `max-image-preview:{none|standard|large}`, `max-video-preview:N`, `unavailable_after:DATE`. Any other token you write passes through as well; the list above is what the merge REDUCES, not an allow-list.

**No token is ever stripped.** Anything the merge does not reduce itself passes through untouched, including tokens no engine reads any more (`nositelinkssearchbox`, `noodp`). Robots meta is specified so that a crawler ignores what it does not read, so an obsolete token costs nothing -- whereas a build-time list of which tokens are dead is a per-vendor judgment no universal module can keep current, and getting it wrong deletes working directives silently. `rel=next`/`rel=prev` pagination links are never emitted (Google dropped support). No `<meta name="keywords">`, ever. No `<base href>`, ever.
...
4. **Generated-image hook:** set `[params.seo] image_partial` to the path of a partial, written exactly as you would pass it to `partial` -- `og-image/card.html` or `og-image/card`, with or without the suffix, resolved under `layouts/_partials/`. The image cascade calls it when a page has no image of its own, and whatever it RETURNS -- a Resource, a slice of Resources, a string path or URL, or nothing -- is normalized like any declared candidate, so a composed card gets the same dimensions and media type, the same JSON-LD treatment, and the same alt chain a page-bundle image gets. Compose at 1200x630 -- the recommended canvas -- and the card is served as the card, uncropped and unduplicated. The partial must `return` its result: a partial that RENDERS markup instead hands back that markup, which names no image, and is reported rather than published.
   ...
   **Precedence:** `seo.images`/`seo.image`/`meta_image`, then `image_params` keys, then native `images`, then `*feature*`/`*cover*`/`*thumbnail*` bundle resources, THEN this hook, THEN `default_image`. So a page that names or bundles its own image keeps it and never carries a generated card in its structured data, while a page that names none gets a card of its own rather than the one banner the whole site shares. The hook is invoked only on pages that reach that tier, so composing an image is never paid for on a page that would discard it.
   ...
   Degradation follows the module's usual contract: a value that is not a single string, a path naming no existing template, and a partial that renders instead of returning each warn once and leave `default_image` to answer. A partial that returns NOTHING is silent -- that is how a generator says it has no template for this page kind, and rendering only whitespace on the branch that declines counts as nothing. Errors INSIDE your partial are yours, and surface as build errors exactly as they do for the two hooks above. Author pages are exempt by design: `Person.image` is resolved without this hook, because a social card is a picture of a page and never a photograph of a human.
...
| Numeric value unparseable (offer prices, `offer_count`, rating values and counts, audience ages, clip offsets, `watch_count`, interaction counts, `number_of_employees`, `merchant_return_days`, shipping `rate`) | `warnf` naming the page and field, deduplicated per field + value + page (two pages with the same bad number each warn) + that single property dropped; node still emits. An explicit 0 is a VALID value nearly everywhere (presence checks test for the key or value being present rather than truthy -- `isset` for numeric table keys, value presence for clip fields, where an explicit null counts as missing -- so a native unquoted `0` -- a free-tier `low_price`, a `worst_rating` of 0, a from-birth `suggested_min_age`, a clip `name` of 0 -- parses and emits, never silently drops). Unquoted native numbers are accepted directly within the parser's bounds: a JSON `2500000` or `0.000001` parses without a warn, but a non-finite float (NaN, an infinity) is rejected, and an integer-context value must fit int64 (range-checked against about 9.2e18). Quoted strings parse as DECIMAL (a `"010"` reads as 10, never octal 8) and cap at fifteen integer digits (plus ten fractional digits in float contexts). Required-value exceptions: an unparseable `seo.offer.price` counts as missing, so SoftwareApplication suppresses its node, Product falls back to the Snippet track gate, and the `product:price:amount` meta is dropped; an unparseable `rating_value` first falls back to the AggregateRating `value` alias when that is separately set, and only when no supplied source parses (Review has no alias) is the WHOLE AggregateRating/Review fragment dropped (never a hollow rating node), so the node-level gates degrade; a clip missing `name`, `start_offset`, or `url`, or whose `start_offset` does not parse, drops that whole clip entry (Google requires all three on every Clip; nothing is fabricated for a missing value) |
...
`test/` holds a Hugo fixture site and Node build-output assertions, run with `bash modules/seo/test/run-tests.sh` (or `run-tests.cmd` on Windows). It builds the fixture thirteen times -- with `[seo.alternates]`, `[seo.links]` and `content_license` unset and then set, under a `baseURL` that carries a path, under that same `baseURL` with `canonifyURLs` on, with those namespaces written as bare scalars, with the kill switch off, with a second language whose params set a noindex baseline, with a two-language site paginating all four kinds that can paginate, with that same site built again under a path-carrying `baseURL`, with `jsonld_container` switched to `"graph"`, with the site's name and its publisher's name set to different strings, with the generated-image hook wired alongside a site default image, and with a home page that declares its own search headline under a site-wide title suffix -- because a surface that is always on is indistinguishable from one that works unless both are checked, a URL that drops the baseURL path is indistinguishable from a correct one at a domain root, `canonifyURLs` makes the whole `relURL` family stop emitting that path while Hugo repairs it afterwards in HTML attributes only -- so a derivation routed through that family stays correct in every other build and breaks `og:url`, `og:image` and the JSON-LD nodes, which no post-processor reaches, a URL pinned to the first pager is indistinguishable from a correct one until a document is served from a pager, the pager resolver's guard compares two values that only diverge once a `baseURL` carries a path, the `@graph` serialization site is reached by no other build, the two ends of the site-name chain are indistinguishable until they resolve to different strings, a hook ranked below the site fallback is indistinguishable from one ranked above it until both are configured at once, and the home page is the one page whose `<title>` does not follow the page title, which no build lacking both a declared headline and a suffix can tell apart. See [`test/README.md`](test/README.md). All partials live under `layouts/_partials/seo/` in three override tiers: RENDERERS at the `seo/` root change WHAT is output, RESOLVERS under `seo/resolve/` change HOW a value is chosen, NODE BUILDERS under `seo/jsonld/` change ONE schema type's shape; `seo/lib/` holds internal cross-cutting utilities.
...
modules/seo/
    ...
    _partials/
      seo/
        ...
        resolve/
          ...
          images.html                      # Returns the ordered slice of normalized image dicts. Takes {page, variant, generated}: `variant` chooses the og crop or the native aspect for all of them, and the optional `generated` payload both enables the seo.image_partial hook and is what that hook receives.
          image.html                       # Returns one normalized image dict {key,resource|url,width,height,type,alt,natural} from a candidate (og crop or natural variant). Answers in resources, never URLs, so nothing is published until something names it.
        ...
        lib/
          ...
          image-url.html                   # Returns the absolute URL of a normalized image dict, publishing that image. The single place a resolved image becomes a file, called where a URL is written.
  ...
  test/                                    # Validation suite: a Hugo fixture site built thirteen times (baseline, configured, subpath, subpath-canonify, badtypes, offswitch, multilingual, pagination, pagination-subpath, graph, sitename, generated, hometitle) plus Node build-output assertions. See test/README.md.
```
