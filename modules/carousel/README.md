...
- **Why it was rejected, reason one -- the placeholder-pseudo-version consumption trap.** This repository's root `CLAUDE.md` documents, for the `modules/pwa` -> `modules/workbox` -> `modules/idb` chain, that inter-module `go.mod` requires which reference PLACEHOLDER pseudo-versions (`v0.0.0-00010101000000-000000000000`) can never be fetched directly -- a plain `hugo mod get` of the top module alone fails with `invalid version: unknown revision 000000000000` for the unresolved sibling, forcing every consumer to add BOTH modules as direct `require` lines pinned to real commit pseudo-versions just to build. That chain tolerates the trap only because it has no alternative: it wraps non-Go-aware upstream JavaScript libraries that must resolve through Go's module graph at all. Carousel has no such excuse -- `modules/images` is a pure Hugo-artifacts module consumed today purely via `[[module.imports]]`, with `go.mod` existing only to satisfy Hugo Modules' Go-module backing requirement, not to express real Go-level coupling. Accepting the same trap here would impose a known consumption cost on every carousel consumer for zero benefit.
...
- **The result:** a consumer who wants only the carousel gets a plain, fully functional, unoptimized-image fallback (see [Progressive enhancement and JavaScript](#progressive-enhancement-and-javascript) and [Usage](#usage)) with a normal `hugo mod get` of one leaf module and no placeholder anywhere in its graph. A consumer who wants the full pipeline imports both modules and gets it for free, with no code duplicated between them.
...
1. **Swap the import.** Replace the foreign module's `[[module.imports]]` entry with this one (and add `modules/images` too if you want the responsive pipeline -- see [Composing with modules/images](#composing-with-modulesimages)), then run `hugo mod get` and `hugo mod tidy`. This module has no placeholder-pseudo-version chain, so a plain `hugo mod get` resolves it directly -- no `hugo.work` trick, no direct-`require` workaround, unlike the [`modules/pwa` chain](../../README.md).
