...
| `perplexity` | link (external) | `https://www.perplexity.ai/search?q=` | `markdown` | Redirects to `/search/new` with the query preserved, then Perplexity's bot filter returns 403 to CLI probes -- real browsers pass. Probed 2026-08-01; the prefill itself is unobserved. |
| `grok` | link (external) | `https://grok.com/?q=` | `markdown` | Returns 200. Probed 2026-08-01; the prefill itself is unobserved. |
| `aistudio` | link (external) | `https://aistudio.google.com/prompts/new_chat?prompt=` | `markdown` | Redirects signed-out visitors to Google sign-in with the full deep link preserved in the `continue` parameter -- the prompt survives the auth flow. Probed 2026-08-01; the prefill itself is unobserved. |
...
**What kind of evidence backs each row**, because "verified" covers two very different things here. Only `claude` has an OBSERVED prefill test: the link was opened in a signed-in browser on 2026-08-02 and the prompt appeared in the composer. The other four rest on a live HTTP probe as of 2026-08-01 plus extraction of Mintlify's deployed convention, which together establish that the URL shape is right and the endpoint answers -- not that the prompt reaches the input box. A status code cannot establish that for any of them, since an unrecognized query parameter is simply ignored and the page still returns 200. Treat the four as convention-backed rather than confirmed, and if you run the signed-in test for one of them, open an issue so this paragraph can be narrowed.
