...
rem The chains are quoted because cmd splits call arguments on commas:
rem unquoted, %~2 receives only "hugo.toml", the rest of the chain lands in
rem arguments nothing reads, and both overlays silently build the base config
rem alone -- which only the canonify control spec can tell from the real thing.
