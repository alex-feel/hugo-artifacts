...
| the answers behind those omissions come from the owning modules | white-box, because the two assertions above would pass just as well if the fixture answered for those formats itself: the hook files live in `agent-readiness` and `search`, and the fixture ships no `_partials/url-retirement/` tree at all |
...
`agentskills` is wired in the merged home list but publishes no document here: every `[[params.agent.skills]]` entry names a remote source the module fetches at build time, and this suite stays offline. The merged-list assertion still covers the format name, which is the part a replacing `[outputs]` table would drop -- and the format being wired while publishing nothing is exactly the condition the publication answers exist for, which is why `/url-manifest.txt` does not list it.
