...
`src` resolves in this order: a `data:` URI or a URL-shaped value is classified first and passes through by default (it is never matched against resources), then a page-bundle resource (exact name, then glob), then a global resource under `assets/` (a leading-slash path is tried against `assets/` first, with the slash stripped), then a leading-slash path that matched no resource resolves as a `static/` path; anything else is a missing source (one warning, raw-src `<img>` fallback). A leading slash means SITE-ROOT-RELATIVE, so a `static/` path is normalized onto the site rather than emitted verbatim: `src="/icon.png"` renders `src="/docs/icon.png"` under `baseURL = "https://example.org/docs/"`, because a file in `static/` publishes under the baseURL path. (The module derives that path from `site.BaseURL` itself, because no Hugo URL function can supply it: `relURL`/`relLangURL`/`absURL` all discard the baseURL path for a value that already begins with `/`, and `relURL` on the slash-stripped remainder stops emitting the path entirely once `canonifyURLs` is on -- which the HTML output post-processor compensates for and other output formats, such as the Markdown twin, never do.) Values carrying their own scheme (`https:`, `mailto:`, ...) and protocol-relative values (`//host/path`) are classified as remote and pass through untouched -- absolutizing those would corrupt them. `page` and `src` are required; `alt` is required unless `decorative` is true.
