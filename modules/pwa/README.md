...
### What this module contributes to a site URL registry

Two of the URLs this module publishes cannot be reached by any walk of `.Site.Pages`, and both are registered: `/sw.js` is a Resource rather than a page, so it is in no page collection because it is not a page at all; and the offline page, which IS a page, carries `build.list = "never"`, which keeps it out of every collection, every feed and the sitemap. Both are stable URLs a deployment really serves, and a 404 at either one is a real defect -- a missing worker disables the whole PWA silently. When a site also imports [`url-retirement`](../url-retirement/README.md), this module registers both with it, so they appear in that module's `/url-manifest.txt` and a deployment check covers them without the site configuring anything. The fingerprinted scripts beside them -- the registration, install-prompt and push bundles -- are deliberately NOT registered: a content-addressed URL changes with its own contents, so listing one would report a change on every rebuild in a file whose only use is showing what genuinely changed. A site that does not import that module is unaffected; the calls are guarded and cost it nothing. Shadowing `layouts/offline/single.html` takes the offline page's registration with it, so a shadowed layout that should stay in the registry makes the same call.
