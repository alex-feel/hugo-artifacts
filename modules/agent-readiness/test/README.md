...
Node build-output assertions for `modules/agent-readiness`, run against the files that twenty Hugo builds publish. The module ships zero JavaScript, so there is no browser behavior to test and the suite carries no Playwright dependency.
...
Both runners validate the shipped data files, perform the repository's pre-launch Hugo process check, build all twenty fixtures, fail hard on any `deprecat`, `ERROR`, or `found no layout file` line in any build log, and then run the assertions.
...
| nobuildtime | `fixture/` | `nobuildtime` | The three per-surface `build_time` switches set false on top of the default configuration. Everywhere else the stamp is on, so this is the only build in which deleting a `build_time` conjunct from the twin builder, `llms.html` or `facts.html` changes a published byte. It also proves the switches govern what is WRITTEN rather than what the public partial returns, and that the twin builder reserves `build_time` in its duplicate-key guard even with the switch off. |
...
| File | Covers |
| --- | --- |
| `tests/00-data.spec.js` | The shipped data files: both TOMLs parse, the registry holds exactly 21 entries with no retired token, every license value ships inert, facts sections carry no `limit` key, the `[llms_index]` table declares nothing but its own `enable` switch (so the two documents of one page walk have nothing to drift on), `[llms]` ships one default flag name and no site-wide `select`, `order` or `limit`, and both i18n files carry the same fifteen keys. |
