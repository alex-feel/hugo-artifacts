...
The index builds and queries in a module Web Worker by default (`worker = true`), so main-thread jank never enters the picture; when worker construction fails (old browsers, blocked CSP, cross-origin asset rewriting), the script transparently falls back to a dynamic `import()` of the same artifact on the main thread. Worker startup is bounded by a boot handshake that covers only script load and evaluation: once the worker acknowledges boot, the index fetch and build take as long as the network honestly needs, so a slow connection never triggers a mid-download fallback that would re-fetch the same index on the main thread. The post-boot phase is deliberately unbounded -- no deadline can tell a slow legitimate download from a hang -- so a stalled fetch is bounded by the browser's own network stack, whose eventual failure surfaces as a `fetch`-phase `search:error`; the accepted residuals, either of which leaves the surface loading, are a fetch a broken service worker parks forever and a hung read-side Cache Storage operation (opening the cache, matching, reading a matched body, or deleting a corrupt entry) while serialized-index caching is active. `new Worker` requires same-origin: consumers rewriting asset URLs to a CDN origin must either keep the worker artifact same-origin or accept the main-thread fallback; a site-side `blob:` wrapper is the escape hatch (it additionally requires `worker-src blob:` in the CSP). The designed-for ceiling is roughly 5,000 pages with the worker; heading sub-records (`headings = true`) raise index size around 1.4x and lower that ceiling accordingly, which is why they are opt-in.
...
| `hotkey` | string | `"mod+k"` | Search hotkey: modifiers from `mod`/`ctrl`/`alt`/`shift` joined with `+`, ending in one key (`mod` = Ctrl on Windows/Linux, Cmd on macOS). Toggles the modal palette; on a page where no palette handles the chord -- there is none, or the palette's own hotkey is disabled per call site and its dialog is closed -- it moves focus into the inline input instead, selecting any existing text (an open palette always blocks the inline branch, because the background is inert, and the inline input advertises the shortcut through `aria-keyshortcuts`). Empty disables; an unparseable value warns once and disables. A hotkey with no non-typing modifier (none of `mod`/`ctrl`/`alt` -- a bare `k`, or shift-only) is suppressed while typing in any field, like `hotkey_slash`, so it cannot eat ordinary keystrokes, close the palette from its own input, or steal focus mid-word. |
| `hotkey_slash` | bool | `false` | Also trigger the hotkey action on `/` -- opening the modal, or focusing the inline input on a page without one (suppressed while typing in any field). |
...
Inbound: dispatching `search:rescan` on `document` re-runs initialization for late-inserted roots (PJAX/Turbo swaps, AJAX-loaded content). Re-scanning with nothing to reconcile touches no DOM -- a stripped `search--enhanced` marker is restored, but an intact one is never re-set, and every control toggle (the modal-trigger sweep included) writes only on an actual state change -- so a host may auto-dispatch rescans from an attribute-observing MutationObserver without feeding that observer a mutation loop. The same write-on-change rule governs the keystroke paths: the state classes, the combobox's `aria-expanded`, and the `?q=` write move only when the current value actually differs, and the active-option markers move only on a real selection change (keyed on the module's own selection record rather than a DOM read) -- so a settled surface's empty or below-minimum keystrokes queue no attribute records and issue no redundant `history.replaceState` calls; a re-adoption that does reconcile a missed navigation mutates the surface in up to three bursts (the synchronous input stomp -- a cold backend's loading paint included -- then the loading clear when the backend readies, then the asynchronous response render; a warm backend skips the middle one), and each of the at most three follow-up rescans those bursts can trigger is DOM-silent. A rescan also re-adopts a reattached page-surface root: a navigation event or another registration occurring while the root sits outside the document drops its `?q=` synchronization (back/forward navigation against a detached root would otherwise drive stale nodes), and the rescan re-registers the same root's sync and reconciles it with the current URL -- a query navigated to while the root was detached is applied at re-adoption rather than waiting for the next navigation -- mirroring the modal's former-owner re-adoption. External `?q=` changes that change nothing -- a re-adoption that missed no navigation, or a history hop or bfcache restore landing on the identical query -- are skipped rather than redundantly re-run, unless the surface is in a per-query error state: an errored surface does not reflect its query, so the identical-query navigation retries it and recovers. After a backend failure, by contrast, the surface deliberately STAYS in the error state: navigations only mirror the URL into the visible input, and typing repaints nothing (explicitly emptying the input still clears `?q=`, so the user's clear survives the reload recovery), because the failed backend stays failed for the page's lifetime and a healthy-looking surface would let later queries die silently -- a form submit's native GET navigation is that path's recovery. One caveat for morphing hosts: a CLONED root copies the `search--enhanced` marker but not the wiring, so a clone is skipped as already enhanced and stays inert; strip the marker class from the clone before dispatching `search:rescan` and it wires fresh.
...
One consumer-side pattern trades this invariant away: replacing the `<noscript>` guidance (by shadowing `layouts/_partials/search/noscript.html` -- the shipped default is itself same-origin, linking only your own per-language `searchindex.json` when the index is wired) with a GET form submitting to an external search engine scoped to your site. That is STRICTLY opt-in, never the default, implemented entirely site-side, and it sends the visitor's query to a third party -- label it as such for your visitors. Similarly, when `show_image` is on and your records reference external thumbnail origins, referencing those origins at all is your privacy decision.
...
│           ├── noscript.html           # shadowable no-JavaScript guidance + JSON-index pointer
│           └── lib/
