...
Inbound: dispatching `search:rescan` on `document` re-runs initialization for late-inserted roots (PJAX/Turbo swaps, AJAX-loaded content). A rescan also re-adopts a reattached page-surface root: its `?q=` synchronization is dropped while the root sits outside the document (back/forward navigation against a detached root would otherwise drive stale nodes), and the rescan re-registers the same root's sync, mirroring the modal's former-owner re-adoption.
