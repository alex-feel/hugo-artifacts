...
// The registered URLs belong to no page and carry no language segment: two name
// files the asset pipeline published and one a file copied from static/, all at
// the publish root, which both languages of a shared-domain site are served
// from.
test('the manifest lists exactly one URL per page of this language', () => {
  assert.deepEqual(manifest(derivedDir, 'en/url-manifest.txt').urls, [
    ...
    '/en/posts/post-one/',
    '/en/tags/',
    '/probe/also-published-by-url-read.txt',
    '/probe/copied-verbatim.txt',
