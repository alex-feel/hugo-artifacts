...
A third instance runs in the ordinary direction and is what a module publishing a page-less URL owes: `url-retirement` ships the PUBLIC `layouts/_partials/url-retirement/register-url.html`, and a module that publishes a URL no walk of `.Site.Pages` can reach -- an asset-pipeline Resource, a page carrying `build.list = never` -- calls it behind the same `templates.Exists` guard so its URL reaches `/url-manifest.txt`. `modules/pwa` is the reference case, registering `/sw.js` and its offline page. Register only what the build really WROTE and only from a template that renders during the html pass: registration is the one arrival path that can ADD a line no file backs, and `/url-manifest.txt` renders at `weight = 100`, so anything recorded from a zero-weight format's own pass arrives too late and is refused. A content-addressed URL is deliberately never registered, because a fingerprinted name changes with its own contents and would report a retirement on every rebuild.

Such a dependency is invisible to every single-module suite -- each fixture imports its own module alone -- so it is proven in `modules/test-composition/`, with a white-box probe of both modules' store keys, and for the publication hooks with a white-box check that the hook files live in the OWNING modules while the fixture ships none. Equality of the published values is NOT sufficient evidence there: those fixtures build in under a second while a stamp's precision is one second, so two independently computed values print the same string.
