...
Such a dependency is invisible to every single-module suite -- each fixture imports its own module alone -- so it is proven in `modules/test-composition/`, with a white-box probe of both modules' store keys. Equality of the published values is NOT sufficient evidence there: those fixtures build in under a second while a stamp's precision is one second, so two independently computed values print the same string.
