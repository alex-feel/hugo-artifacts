...
Hugo aborts any page whose render exceeds the site-level `timeout` setting (default `60s`), and build-time poster fetching counts toward the clock of the page being rendered. The ceilings below are per call site. During a trip-eligible outage the first call site costs roughly one 30s request window before the breaker trips -- up to about ten seconds longer when the CDN answers with the statuses Hugo retries internally (408, 429, 500, 502, 503, 504), because the final backoff sleep does not observe the request deadline. A CDN that stays reachable but slow -- eventually answering each tier with a status code -- never trips the breaker, so one call site can legitimately spend up to roughly 90s walking all three tiers in sequential request windows, and a CDN that degrades mid-walk (slow status-carrying tiers followed by a status-less one) can reach roughly 100s before the breaker finally trips. Untripped walks also stack: embeds share their page's render clock, so several embeds against a slow status-answering CDN can each spend up to roughly 90s in sequence. Budget the site-level margin accordingly -- `timeout = '180s'` absorbs one full untripped walk with room for the rest of the page, and each additional embed on the same page wants roughly 90s more headroom -- to keep outage behavior a warning story instead of a build failure.
