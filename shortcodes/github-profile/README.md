...
| `org-rollup` | Per-organization contribution rollup: commits, issues, pull requests, reviews per owner, each row led by the owner's avatar |
...
| `orgs` | Public organization memberships, each led by the organization's avatar |
...
| `org-rollup-limit` | int | no | `10` | How many rollup rows render, top-N by total; `0` renders every row. A truncated list carries its pre-cap total in `data-total` and closes with a visible localized remainder note |
| `org-rollup-owners` | string | no | `all` | Which owner types the rollup keeps: `all` (organizations and user accounts alike) or `organizations` (rows owned by individual user accounts are dropped before the cap, so `data-total` and the remainder note count only kept owners). The list names the selection in `data-owners` either way |
| `contributed-limit` | int | no | `12` | How many contributed repositories render, top-N by stars; `0` renders every fetched one -- the remainder note then reflects only what the API withheld past its 100-per-connection ceiling |
| `avatar` | string | no | `fetch` | Avatar handling for the identity avatar and the organization avatars in the rollup and membership lists: `fetch` (build-time copies, placeholder on failure), `hotlink`, `none` |
...
- Passing an invalid `variant`, `history`, `avatar`, `language-scope`, or `org-rollup-owners` value fails the build with an error message.
- Passing a non-integer or negative `org-rollup-limit` or `contributed-limit` fails the build with an error message; `0` is valid and means every row.
...
| `avatar="fetch"` | +1 image GET to GitHub's avatar CDN per avatar rendered -- the identity avatar when `identity` is active, plus one per distinct organization in the rollup and membership lists (not the API budget; a distinct avatar is fetched once per build however many surfaces render it) |
...
- **Avatar fetch failure:** the placeholder icon renders instead of the image -- the person glyph for the identity avatar, the organization glyph for an organization's -- with one warning per distinct avatar however many surfaces render it. An organization whose data carries no avatar URL at all gets the same placeholder silently, because that is an API answer rather than a fetch failure.
...
- **Per-organization rollup** groups the four per-repository contribution lists by repository owner (the person's own account excluded) into per-owner commits, issues, pull requests, and reviews, each row led by the owner's avatar. Scoped to the contribution window and to the API's 100-repositories-per-type ceiling, and capped to the top `org-rollup-limit` owners by total (10 by default); the pre-cap owner count is published on the list and a visible note names how many rows the cap cut. `org-rollup-owners="organizations"` drops user-owned rows before any of that counting, so the total, the cap, and the note all describe only the owners the list actually shows.
...
- **The headline's organizations metric and the rollup's row count answer different questions.** The metric counts distinct organizations among all repositories the person has contributed to (`repositoriesContributedTo`, all four contribution types); the rollup groups the contribution window's per-repository lists by owner -- organizations and user accounts alike under the default `org-rollup-owners="all"`, organization-owned rows only under `organizations` -- and keeps the top rows its limit allows. A profile can legitimately show `12 organizations` above a shorter rollup: some of those organizations saw their contributions outside the window or past the per-type ceiling, and some rows may be cut by the limit -- which the list's `data-total` and its visible remainder note account for, so the cut part is never silent.
...
| `github_profile_more` | `and {{ .formatted }} more` (plural forms) | Visible remainder under a truncated list; deliberately unitless, because the list above it names the unit |
...
Each list section (`org-rollup`, `languages`, `contributed`, `orgs`, `pinned`, `socials`) opens with a `github-profile__section-title` paragraph carrying the section's localized label and a `data-section` token, so a site styles real text instead of fabricating a heading from the list's `aria-label` with `attr()`. It is a neutral `<p>`, not a heading element, because the module cannot know the host page's outline level. It is `aria-hidden` because it restates verbatim the accessible name the adjacent list already carries; that is the module's rule for pure restatements (this title, the calendar legend), while text adding information its neighbor does not convey stays announced (the calendar summary). Hide it with CSS if the site supplies its own heading. A truncated rollup, contributed, or membership list closes symmetrically with a `github-profile__more` paragraph carrying the visible localized remainder (`and N more`), its `data-section` token, and the machine-readable count in `data-raw`; it stays announced, because unlike the title it adds information the list does not carry.

Each rollup row and membership entry leads with the organization's avatar inside the name anchor: an `<img>` (`github-profile__org-roll-avatar` / `github-profile__org-avatar`, intrinsic 64x64, `loading="lazy" decoding="async"`, empty `alt` because the adjacent text carries the name) when the data names one, or the generic organization glyph inside a `--placeholder` span of the same class when it does not. Under `avatar="none"` the membership entry keeps the bare glyph and the rollup row keeps the bare name. The avatars are sized by the consuming site's CSS like everything else; the intrinsic attributes only reserve layout space. In the identity section, `github-profile__identity-body` wraps every non-avatar field, so an avatar-beside-text layout is a single flex rule. In the attribution line, the icon, the `@login`, and the localized suffix are separate elements inside one anchor, so a site can split them across the line (for example, `display: flex; justify-content: space-between` on the anchor) or hide the suffix for a bare handle.
...
| `data-total` | calendar, rollup, contributed and orgs lists | Total contributions / the list's true total before any cut (the rollup's pre-cap owner count, the contributed connection's `totalCount`, the membership `totalCount`) |
| `data-raw` | calendar summary total | Unformatted integer total (the visible text is locale-formatted) |
| `data-raw` | remainder notes | How many entries the list did not render |
...
| `data-section` | list-section titles, remainder notes | Section token: any of `org-rollup`, `languages`, `contributed`, `orgs`, `pinned`, `socials` on a title; only `org-rollup`, `contributed`, or `orgs` on a remainder note, because the other lists never truncate |
| `data-owners` | rollup list | Which owner types the rollup keeps: `all` or `organizations` (the `org-rollup-owners` parameter, echoed so the claim always matches the rows) |
...
| `data-avatar` | org and rollup items | Organization avatar URL, the CDN original -- under `avatar="fetch"` the rendered img's `src` names the build-time copy instead, so the two are deliberately distinct |
