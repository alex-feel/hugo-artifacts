...
arXiv reports "not found" with an HTTP 200 body (an empty feed for a nonexistent id, or an error `entry` for a malformed id), so `parse-atom.html` inspects the feed shape rather than trusting the status code. On retry exhaustion the module emits a single structured `warnf` -- carrying a `Hint:` field that names a wait figure (clamped to one day) for rate-limit failures and points to the warning context otherwise, matching the sibling modules; the once-per-build `media-type-config` warn is the one exception, since its message already names the exact configuration fix -- and falls through to graceful degradation. The build is never broken by an API failure.
