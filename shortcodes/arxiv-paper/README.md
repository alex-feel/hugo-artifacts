...
arXiv reports "not found" with an HTTP 200 body (an empty feed for a nonexistent id, or an error `entry` for a malformed id), so `parse-atom.html` inspects the feed shape rather than trusting the status code. On retry exhaustion the module emits a single structured `warnf` -- always carrying a `Hint:` field, which names a wait figure (clamped to one day) for rate-limit failures and points to the warning context otherwise, matching the sibling modules -- and falls through to graceful degradation. The build is never broken by an API failure.
