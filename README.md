...
Universal agent-readiness module: the agent-facing representations of a site your HTML already describes, all driven by one shared page-selection filter so they can never disagree -- a generated `robots.txt` with a 21-token AI-crawler registry and a `Content-Signal` declaration, per-page Markdown twins built from `.RenderShortcodes` with a duplicate-key-safe YAML front matter, a compact `llms.txt` link index beside a complete `/llms-index.txt` one -- two documents from ONE page walk, so a per-section selection principle can bound the file an agent reads first without putting anything out of reach, and a section that principle narrowed opens with a line saying how many pages it is showing and where the rest are, so a reader cannot mistake an omission for a fact about the site -- an aggregated `/about.md` facts document that is complete by construction rather than truncated, and a `/.well-known/agent-skills/` discovery index whose SHA-256 digests are computed from the exact bytes it republishes -- single SKILL.md bodies, whole archives, and the declared supporting files it republishes beside a multi-file skill's SKILL.md so their relative references resolve. The twins, both link indexes, `/about.md` and the skills index each carry one build stamp -- a single RFC 3339 timestamp per build, byte-identical across every document and every language, and exposed through a public partial so a consuming site can stamp its own surfaces, and the `search` module its index, with the same string. It answers what a digest cannot: a hash says whether two copies differ, never which one is newer. Zero CSS, zero JavaScript, zero visual surface; a four-tier configuration cascade under `params.agent`; graceful degradation on every misconfiguration and every failed remote fetch. See [`modules/agent-readiness/README.md`](modules/agent-readiness/README.md).
...
Universal build-time Open Graph card generator: one `{{ partial "og-image/card.html" . }}` call returns a composed social card for the page as an image resource, or nothing when the page is one this generator has no template for. A card is a site-supplied background raster normalized to the canvas, any number of overlays, and any number of text slots drawing the page's own words or the site's canonical facts -- title, description, section, date, a front-matter param, a data-file value, a literal -- laid out by a text engine that measures and wraps against a configured box, walks a bounded size ladder when the words do not fit, and truncates with an ellipsis only at the floor. The module ships no templates, no fonts and no colors: the consuming site supplies the backdrop, the coordinates and the type, so every card is that site's design rather than this module's. PNG, JPEG or WebP output, a four-tier configuration cascade under `params.ogcard`, one card per page and language memoized across the repeated calls a page receives, and graceful degradation throughout -- an unreadable font, an unusable background or a failed compose is one deduplicated warning and a page that keeps whatever image it would otherwise have had, never a broken build. Composes with `modules/seo`: point `[params.seo] image_partial` at `og-image/card.html` and the composed card becomes the page's `og:image`, with real dimensions and a matching JSON-LD image node, while a page the generator declines falls through to the site's default image. It also ships a standalone `{{ partial "og-image/meta.html" . }}` renderer for sites that do not use the seo module. See [`modules/og-image/README.md`](modules/og-image/README.md).

### `modules/pwa`

Consumer-facing Progressive Web App module: web app manifest, RealFaviconGenerator icon set, Workbox-powered service worker, install prompt, and push subscription wiring. Composes with [`modules/url-retirement`](modules/url-retirement/README.md): the two URLs it publishes that no walk of a site's pages can reach -- the service worker and the offline page -- are registered with that module's URL manifest when a site imports both, so a deployment check covers them without the site naming either by hand. See [`modules/pwa/README.md`](modules/pwa/README.md).
...
Universal URL-retirement module: publishes `/_redirects` -- the host redirect map read by Cloudflare Pages, GitLab Pages and Netlify -- containing the site's own hand-written rules verbatim followed by one real `301` redirect per page alias, one per paginated list and one for the redirect Hugo runs between the site root and its default language, so a retired URL -- including the `/blog/page/1/` Hugo stops publishing once its pagination alias is switched off, and the `/en/` or `/` a multilingual site retires -- becomes a server-side redirect instead of a meta-refresh stub, plus a per-language `/url-manifest.txt` listing every URL the build publishes -- every page crossed with its output formats, plus paginated list pages, whatever a module registers for what it published outside the page graph, and whatever a format's owner answers when the manifest asks what else that format wrote, none of which a sitemap contains -- so a deployment check can diff what production serves against what the build produces instead of diffing a sitemap, which is a filtered subset. Style-agnostic, zero CSS, zero JavaScript, a two-tier configuration cascade under `params.url_retirement`, and graceful degradation on every misconfiguration -- the one fatal case is an alias containing whitespace, which would silently corrupt the file format. See [`modules/url-retirement/README.md`](modules/url-retirement/README.md).
